...
int main()
{
    ...
    int N = 1000;
    //lambda 500nm
    double l = 10 * 500 / pow(10, 9);
    //x with lambda and r +- 25 micro meters
    double equation = (M_PI * 25 / (pow(10, 6)));
    double beginning = -equation / l;
    double ending = equation / l;

    double* x_range = linspace(beginning, ending, N);
    ...
    putchar('\n');
    FILE* fp = fopen("data1b_part1.txt", "w");
    if (!fp)
    {
